...
def add_app(application):
    ...
    #DONE
    users[application] = {"password": generate_password_hash(cred), "role": "funcbeh"}
    ...
    return make_response(render_template('succes.html', result=application), 200)
...
@app.route('/<app>/func', methods=['POST'])
#TODO: check that logged in user is sysop
#DONE
@auth.login_required(role='sysop')
